...
Returns plain Python types only (no numpy) so FastAPI can serialise.
"""
import logging
...
def _r(v, d=5):
    """Round to d decimals, return plain Python float."""
    try:
...
def calc_ema(closes, period):
    ...
    for c in closes[period:]: ema = float(c)*k + ema*(1-k)
    return round(ema, 6)

def calc_atr(candles, period=14):
    if len(candles) < period+1: return None
    trs = [max(float(c["high"])-float(c["low"]),
...
def circuit_breaker_active(oanda_client):
    try:
        ...
        return {"active": breached, "loss": round(today_loss, 2),
                "limit": round(limit, 2), "balance": round(balance, 2),
...
class SignalEngine:
    ...
    def analyse(self, instrument):
        try:
                    ...
                    if news.get("blocked"):
                        return {
                            ...
                            "reasons": [
                                ...
                                "Trading blocked ±30 mins around release",
            ...
            if bs >= 45 and bs > ss:
                signal = "BUY"; confidence = min(95, int(bs)); reasons = br
                try:    entry = float(self.client.get_live_price(instrument)["ask"])
                except: entry = price
                if atr:
                    sl = _r(entry - atr*1.5, 5)
                    tp = _r(entry + atr*2.5, 5)
            elif ss >= 45 and ss > bs:
                signal = "SELL"; confidence = min(95, int(ss)); reasons = sr
                try:    entry = float(self.client.get_live_price(instrument)["bid"])
                except: entry = price
                if atr:
                    sl = _r(entry + atr*1.5, 5)
                    tp = _r(entry - atr*2.5, 5)
